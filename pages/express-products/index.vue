      ...
      <div>
              ...
              <HeadlessTransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                ...
                <HeadlessDialogPanel
                  ...
                  <form class="mt-4 border-t border-gray-200">
                    <!-- Price -->
                    <HeadlessDisclosure as="div" class="px-4 py-6 border-t border-gray-200" v-slot="{ open }">
                      <h3 class="flow-root -my-3">
                        <HeadlessDisclosureButton
                          ...
                          <span class="flex items-center ms-6">
                            <icon name="ant-design:plus-outlined" class="w-5 h-5" aria-hidden="true" v-if="!open" />
                            <icon name="ant-design:minus-outlined" class="w-5 h-5" aria-hidden="true" v-else />
                          </span>
                        </HeadlessDisclosureButton>
                      </h3>
                      <HeadlessDisclosurePanel class="pt-6">
                        <div class="space-y-4">
                          <form class="relative flex items-center space-s-1" @submit.prevent="filterProducts">
                            <div>
                              <div class="relative mt-1 rounded-md shadow-sm">
                                <input id="minPrice" name="minPrice" v-model="minPrice"
                                  ...
                                  type="number" step="100" :placeholder="$t('placeholder.min')">
                              </div>
                            </div>
                            ...
                            <div>
                              <div class="relative mt-1 rounded-md shadow-sm">
                                <input id="maxPrice" name="maxPrice" v-model="maxPrice"
                                  class="flex-1 block w-full px-2 py-2 placeholder-gray-400 border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 h-9"
                                  step="100" type="number" :placeholder="$t('placeholder.max')">
                              </div>
                            </div>
                            <button
                        ...
                        <filteration>
                          <template #fulfillment="{ plusIcon, minusIcon, label }">
                            <HeadlessDisclosureButton
                              ...
                              <span class="flex items-center ms-6">
                                <icon :name="plusIcon" class="w-5 h-5" aria-hidden="true" v-if="!open" />
                                <icon :name="minusIcon" class="w-5 h-5" aria-hidden="true" v-else />
                              </span>
                            </HeadlessDisclosureButton>
                          </template>
                        </filteration>
                      ...
                      <HeadlessDisclosurePanel class="pt-6">
                        <div class="space-y-4">
                          <div v-for="color in colors" :key="color.value" class="flex items-center">
                            <input id="color" name="color" :value="color.label" type="checkbox" :checked="color.checked"
                              class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
        ...
        <main class="max-w-full px-4 mx-auto sm:px-6 lg:px-8">
          <div class="flex items-baseline justify-between pt-12 pb-6 border-b border-gray-200">
            <p>1 - 55 {{ $t('products.pages_over') }} 1,319 {{ $t('products.items_for') }} <span
                class="font-semibold tracking-tight text-blue-700 underline">express</span></p>
              ...
              <form class="hidden lg:block">
                ...
                <HeadlessDisclosure as="div" class="py-6 border-b border-gray-200" v-slot="{ open }">
                  ...
                  <HeadlessDisclosurePanel class="pt-6">
                    <div class="space-y-4">
                      <div v-for="ful in fulfillment" :key="ful.value" class="flex items-center">
                        <input id="category" name="category" :value="ful.label" type="checkbox" :checked="ful.checked"
                          class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
...
const fulfillment = ref([
  { value: "express deals", label: "Express Deals", checked: true },
])
