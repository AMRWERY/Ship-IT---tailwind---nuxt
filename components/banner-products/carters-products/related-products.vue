  ...
  <div>
    ...
    <swiper :spaceBetween="30" :centeredSlides="true" :autoplay="{ delay: 2500, disableOnInteraction: false }"
      ...
      <swiper-slide v-for="product in storeTwo.products" :key="product">
        <div class="relative m-10 w-[300px] max-w-[16rem] overflow-hidden rounded-2xl bg-white shadow-md dark:bg-neutral-700">
          <div>
          ...
          <div class="px-5 pb-5 mt-4">
            ...
            <div class="flex items-center justify-between">
              <p>
                <span class="text-lg font-bold text-red-600 me-2 dark:text-red-200">{{ $t('products.egp') }} {{ product.price
                  }}</span>
                <span class="line-through text-md text-slate-900 dark:text-slate-300" v-if="product.originalPrice">{{ $t('products.egp')
                  }} {{
                    product.originalPrice
                  }}</span>
              </p>
            </div>
            <p class="mt-2 text-sm text-start text-slate-700 dark:text-slate-300">{{ $t('products.get_it_by') }} {{ product.getBy }}</p>
          </div>
        </div>
      </swiper-slide>
    </swiper>
